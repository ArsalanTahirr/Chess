...
Piece::Piece(Color color, const char character, const Position& position) :
    color(color), position(position), isCaptured(false), character(character),
    symbolForBoard(getPieceSymbol(character)), type(PieceType::None),
    file('a' + position.X), rank('1' + position.Y) {
